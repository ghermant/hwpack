...
void do_sdrc_init(u32 cs, u32 early)
{
	...
	if (cs) {
		...
		writel(readl(&sdrc_base->cs[CS0].rfr_ctrl), &sdrc_base->cs[CS1].rfr_ctrl);
		writel(readl(&sdrc_actim_base0->ctrla), &sdrc_actim_base1->ctrla);
		writel(readl(&sdrc_actim_base0->ctrlb), &sdrc_actim_base1->ctrlb);
	}
